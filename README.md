...
В конструктор передается базовый адрес сервера и глобальные опции для всех запросов(опционально): `constructor(baseUrl: string, options: RequestInit = {})`.  
Методы: 
...
В конструктор передается карта для хранения событий и подписок: `constructor() {
...
Конструктор принимает для хранения данные объекта и эмиттер событий: `constructor(data: Partial<T>, protected events: IEvents)`.  
...
Конструктор доступен в пределах самого класса и его подклассов и принимает родительский HTML-элемент, в котором будет размещен компонент: `protected constructor(protected readonly container: HTMLElement);`.  
Методы:
...
Конструктор принимает родительский элемент и обработчик событий: `constructor(container: HTMLElement, protected events: IEvents)`.  
